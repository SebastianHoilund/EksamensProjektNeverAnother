...
import com.example.eksamensprojekt_neveranother.data.MeasurementRepository
...
class MeasurementViewModel : ViewModel() {
    ...
    fun completeMeasurement() {
        isTailored = true
        // Gem målingerne til Supabase når processen er færdig.
        viewModelScope.launch {
            try {
                repository.saveMeasurement(measurement)
            } catch (e: Exception) {
                // Her kunne man logge fejlen eller vise en besked til brugeren.
                e.printStackTrace()
            }
        }
    }
